...
public class SpeechToText extends BaseService {
  ...
  public static SpeechRecognitionResults fromSpeechToText(File audio) {
	  
	  String check = new String("Ha entrado en Speech to text");
	  Authenticator authenticator = new IamAuthenticator("aXGQInQiqn34_VVMcrKsaBWrtEIE7X45SZWYB9jQNPiy");
	  SpeechToText service = new SpeechToText(authenticator);

	  check = new String("Ha pasado el authenticato");

	  RecognizeOptions options = null;
	try {
		check = new String("principio del try");
		options = new RecognizeOptions.Builder()
		    .audio(audio)
		    .contentType(HttpMediaType.AUDIO_WAV)
		    .build();
		check = new String("final del try");
	} catch (FileNotFoundException e) {
		// TODO Auto-generated catch block
		e.printStackTrace();
		check = new String("no file found");
	}
